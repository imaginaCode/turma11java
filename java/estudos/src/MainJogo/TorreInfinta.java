...
public class TorreInfinta {
			...
			switch (menuP) {
				...
				case 2 :
					if (chave) {
						...
						System.out.println("Você ainda nao encontrou a chave, Continue Procurando :)");
						Thread.sleep(1000);
						limpaTela();
					}
	...
	public static void abrirBau() throws InterruptedException{
		...
		{
			 // pedra
			System.out.println("Algo está brilhando no bau!!!!");
			imprimirPedra();
			...
			if (valor2 <7) {
				...
				System.out.println((saidaPG[0])==1? "Pedra da força":(saidaPG[0])==2? "Pedra Vitalidade" : (saidaPG[0])==3? "Pedra Defesa":(saidaPG[0])==4? "Pedra Magia":"Pedra da Agilidade" );
